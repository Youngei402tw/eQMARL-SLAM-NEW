...
mode="${1:-fast}"
if [[ "$mode" != "fast" && "$mode" != "pilot" && "$mode" != "full" ]]; then
    echo "Usage: bash scripts/submit_vanda_four.sh [fast|pilot|full]" >&2
    exit 2
fi

for framework in eqmarl_psi+ qfctde fctde sctde; do
    variables="SLAM_FRAMEWORK=$framework,SLAM_MODE=$mode"
    for name in SLAM_N_EPISODES SLAM_MAX_STEPS SLAM_SEED; do
        value="${!name-}"
        if [[ -n "$value" ]]; then
